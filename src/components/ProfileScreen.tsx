  ...
  return (
    <div className="flex-1 flex flex-col bg-slate-50 text-slate-900 overflow-y-auto h-full" id="profile-screen-container">
          ...
          <form onSubmit={handleSaveChanges} className="p-5 space-y-4">
            ...
            <div className="space-y-3 bg-slate-50 p-4 rounded-2xl border border-slate-100">
              ...
              <div className="pt-2">
                ...
                >
                  <Compass size={13} className={editLat && editLng ? "text-indigo-600 animate-spin-slow" : "text-slate-500"} />
                  {editLat && editLng ? (
      ...
      {activeSubScreen === "my_listings" && (
        <div className="flex-1 flex flex-col animate-fade-in bg-slate-50" id="profile-sub-my-listings">
          {/* Sub Header */}
          <div className="flex items-center gap-3 px-4 py-4 border-b border-slate-100 sticky top-0 bg-white z-10">
            <button
              onClick={() => setActiveSubScreen("menu")}
          ...
          <div className="p-4 space-y-3.5 flex-1 pb-16">
            {myParts.length === 0 ? (
              <div className="text-center py-16 px-6 bg-white rounded-3xl border border-slate-100 shadow-sm mt-4">
                ...
                <h4 className="text-xs font-bold text-slate-700">No Listings Yet</h4>
                <p className="text-[10px] text-slate-400 mt-1.5 max-w-[200px] mx-auto">
                  You have not listed any automobile spare parts for sale. Go to the "Sell" tab on the bottom menu to list one.
                </p>
              </div>
            ) : (
                          ...
                          >
                            <div className="relative flex-1">
                              ...
                              <input
                                ...
                                className="w-full bg-slate-50 border border-slate-200 rounded-xl py-1 pl-4 pr-1 text-xs font-mono font-extrabold focus:outline-none focus:border-indigo-500"
                                required
                                min="0"
                                autoFocus
                              />
                            ...
                            <button
                              onClick={() => {
                                ...
                                if (!isNaN(num) && num >= 0) {
                                  onUpdatePrice && onUpdatePrice(part.id, num);
                                  setEditingPriceId(null);
                                }
                              }}
                              className="bg-indigo-600 hover:bg-indigo-500 text-white font-extrabold px-2 py-1 rounded-xl text-[9px] uppercase tracking-wider"
                            >
                              Save
                            </button>
                            <button
                              onClick={() => setEditingPriceId(null)}
                              className="bg-slate-100 hover:bg-slate-200 text-slate-600 font-bold px-2 py-1 rounded-xl text-[9px] uppercase tracking-wider"
                            >
                              Cancel
                            </button>
                          </div>
                          ...
                          <div className="flex items-center justify-between gap-2">
                            ...
                            <button
                              onClick={(e) => {
                                ...
                                setEditPriceValue(part.price.toString());
                              }}
                              className="text-[10px] text-indigo-600 hover:text-indigo-500 font-black uppercase tracking-wider hover:underline"
                            >
                              Edit Price
                            </button>
                          </div>
                    ...
                    {deleteConfirmId === part.id ? (
                      <div className="border-t border-slate-50 pt-2.5 flex flex-col gap-2 w-full animate-fade-in">
                        <p className="text-[10px] font-extrabold text-rose-600 leading-tight">
                          Delete listing permanently from Firestore?
                        </p>
                      ...
                      <div className="border-t border-slate-50 pt-2.5 flex items-center justify-between">
                        {/* Functional MARK AS SOLD Toggle Button */}
                        <button
        ...
        <div className="flex-1 flex flex-col animate-fade-in bg-slate-50" id="profile-sub-support">
          ...
          <div className="p-4 space-y-4">
            ...
            <div className="bg-white rounded-3xl p-5 border border-slate-100 shadow-sm space-y-3">
              <h3 className="text-xs font-extrabold text-slate-800">Direct Support Actions</h3>
              <p className="text-[10px] text-slate-500">
                Get assistance with your trades or listings from the Auto Parts helper team:
              </p>

              <div className="space-y-2 pt-1">
                {/* Email Support */}
                <a
                  href="mailto:support@autoparts.in?subject=Auto%20Parts%20Inquiry"
                  className="w-full flex items-center justify-between p-3 bg-slate-50 hover:bg-indigo-50/50 rounded-2xl border border-slate-100 text-slate-700 transition-all group"
                >
                  <div className="flex items-center gap-2.5">
                    <span className="p-1.5 bg-indigo-50 text-indigo-600 rounded-lg group-hover:bg-indigo-100">
                      <Mail size={12} />
                    </span>
                    <span className="text-[11px] font-bold">Email Support</span>
                  </div>
                  <span className="text-[10px] text-slate-400 font-mono group-hover:text-indigo-600 flex items-center gap-0.5">
                    support@autoparts.in <ExternalLink size={10} />
                  </span>
                </a>

                {/* WhatsApp Help */}
                <a
                  href="https://wa.me/919876543210?text=Hello%20Auto%20Parts%20Support%2C%20I%20need%20help%20with%20my%20listings."
                  target="_blank"
                  rel="noopener noreferrer"
                  className="w-full flex items-center justify-between p-3 bg-slate-50 hover:bg-emerald-50/50 rounded-2xl border border-slate-100 text-slate-700 transition-all group"
                >
                  <div className="flex items-center gap-2.5">
                    <span className="p-1.5 bg-emerald-50 text-emerald-600 rounded-lg group-hover:bg-emerald-100">
                      <MessageSquare size={12} />
                    </span>
                    <span className="text-[11px] font-bold">WhatsApp Support</span>
                  </div>
                  <span className="text-[10px] text-slate-400 font-mono group-hover:text-emerald-600 flex items-center gap-0.5">
                    +91 98765 43210 <ExternalLink size={10} />
                  </span>
                </a>
                ...
                <a
                  ...
                  className="w-full flex items-center justify-between p-3 bg-slate-50 hover:bg-sky-50/50 rounded-2xl border border-slate-100 text-slate-700 transition-all group"
                >
                  <div className="flex items-center gap-2.5">
                    <span className="p-1.5 bg-sky-50 text-sky-600 rounded-lg group-hover:bg-sky-100">
                      <PhoneCall size={12} />
                    </span>
                    <span className="text-[11px] font-bold">Call Helpline</span>
                  </div>
                  <span className="text-[10px] text-slate-400 font-mono group-hover:text-sky-600 flex items-center gap-0.5">
                    Mon - Sat (9 AM - 6 PM) <ExternalLink size={10} />
                  </span>
                </a>
              </div>
            </div>
            ...
            {/* FAQs */}
            <div className="bg-white rounded-3xl p-5 border border-slate-100 shadow-sm space-y-3">
              <h3 className="text-xs font-extrabold text-slate-800">Frequently Asked Questions</h3>
              
              <div className="space-y-3.5 divide-y divide-slate-50 pt-1">
                <div className="space-y-1">
                  <h4 className="text-[11px] font-bold text-slate-800">Is listing spare parts free of charge?</h4>
                  <p className="text-[10px] text-slate-500 leading-normal">
                    Yes, listing auto spare parts for sale on Auto Parts is completely free with zero commissions or premium listing packages.
                  </p>
                </div>

                <div className="space-y-1 pt-2.5">
                  <h4 className="text-[11px] font-bold text-slate-800">How do I negotiate or purchase listed parts?</h4>
                  <p className="text-[10px] text-slate-500 leading-normal">
                    You can tap the active listing card and click "In-App Chat" to chat with the seller in real-time, or call them directly.
                  </p>
                </div>

                <div className="space-y-1 pt-2.5">
                  <h4 className="text-[11px] font-bold text-slate-800">What is the "Mark as Sold" feature?</h4>
                  <p className="text-[10px] text-slate-500 leading-normal">
                    Once a spare part is successfully sold, you can toggle its status. This will show a SOLD label to buyers so they stop calling or messaging about it.
                  </p>
                </div>
              </div>
            </div>
