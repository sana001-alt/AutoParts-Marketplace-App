...
export default function ChatRoomWindow({ chat, currentUser, onClose }: ChatRoomWindowProps) {
  ...
  const handleSend = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Trigger simulated context-aware buyer/seller reply after 2.5 seconds
      setTimeout(async () => {
        const textLower = messageText.toLowerCase();
        let reply = "Got it! Let me know if you need any other details or if you want to inspect the part in person.";
        
        if (textLower.includes("available") || textLower.includes("still have")) {
          reply = `Yes, the ${chat.partTitle.toLowerCase()} is still available! It is in clean condition. When would you like to inspect it?`;
        } else if (textLower.includes("price") || textLower.includes("negotiable") || textLower.includes("discount") || textLower.includes("rs") || textLower.includes("rupees") || textLower.includes("cheaper") || textLower.includes("offer")) {
          reply = `The price is slightly negotiable for a serious buyer. What is your offer?`;
        } else if (textLower.includes("condition") || textLower.includes("scratch") || textLower.includes("damage") || textLower.includes("wear")) {
          reply = "The component is in superb condition, exactly as pictured. Tested and fully functional with normal cosmetic wear.";
        } else if (textLower.includes("where") || textLower.includes("location") || textLower.includes("address") || textLower.includes("meet")) {
          reply = "I'm located near the local market. We can meet in the central parking lot. Where are you located?";
        } else if (textLower.includes("compatible") || textLower.includes("fit") || textLower.includes("model") || textLower.includes("year")) {
          reply = "It fits the models mentioned in the listing. You can verify against the OEM part number just to be absolutely sure!";
        } else if (textLower.includes("hello") || textLower.includes("hi") || textLower.includes("hey")) {
          reply = `Hello! Are you interested in purchasing the ${chat.partTitle.toLowerCase()}? Let me know if you have any questions!`;
        } else {
          const genericReplies = [
            "Sounds good! Let me check my garage and keep you updated.",
            "Understood. Let me know when you'd like to schedule a pickup or delivery.",
            "Sure, let's connect soon to finalize the details.",
            "Perfect. I am ready when you are."
          ];
          reply = genericReplies[Math.floor(Math.random() * genericReplies.length)];
        }

        try {
          // If current user is buyer, sender of simulated reply is seller (and vice-versa)
          const senderId = currentUser.id === chat.buyerId ? chat.sellerId : chat.buyerId;
          await sendChatMessage(chat.id, senderId, reply, chat);
        } catch (simErr) {
          console.error("Failed to deliver simulated reply:", simErr);
        }
      }, 2500);
